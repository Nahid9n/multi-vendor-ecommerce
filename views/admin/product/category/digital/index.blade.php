...
@section('title','digital category page')
@section('body')
                ...
                <div class="border-bottom m-3">
                    <div class="row">
                        <div class="card-header border-bottom justify-content-between">
                            <h3 class="card-title text-bold">Digital Category Table</h3>
                            <a href="{{route('digitals.create')}}" class="btn btn-primary my-1 text-center">+ADD</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                            ...
                            @foreach($categories as $key=>$category)
                                <tr>
                                        ...
                                        <form action="{{route('admin.category.featured.status.update',$category->id)}}" class="main-toggle-group" method="post">
                                            ...
                                            <div class="row justify-content-center">
                                                <div class=" text-center col-4 p-2 rounded-5">
                                                    <input id="feature_product{{$key}}" class="toggle toggle-secondary my-1 on " value="1" name="featured_status" {{$category->featured_status == 1 ? 'checked':''}} onclick="this.form.submit()" type="checkbox"/>
                                    ...
                                    <td class="d-flex text-center">
                                        <a href="{{route('digitals.edit',$category->id)}}" class="btn btn-success my-2 me-1"><i class="fa fa-edit"></i></a>
                                        <form action="{{route('digitals.destroy',$category->id)}}" method="post">
                                            @csrf
                                            @method('DELETE')
