                            ...
                            @foreach($manageSliders as $slider)
                                <tr class="text-center">
                                    ...
                                    <td class="d-flex text-center">
                                        <a href="{{route('slider.show',$slider->id)}}" class="btn btn-primary mb-1 mx-1"><i class="fa fa-regular fa-eye"></i></a>
                                        <a href="{{route('slider.edit',$slider->id)}}" class="btn btn-primary mb-1"><i class="fa fa-regular fa-edit"></i></a>
                                        <form action="{{ route('slider.destroy', $slider->id) }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" onclick="return confirm('are you sure to delete ?')" class="btn btn-danger mx-1"><i class="fa fa-regular fa-trash-o"></i></button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
