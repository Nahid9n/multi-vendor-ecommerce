...
@section('body')
    ...
    <div class="page-header">
        <div>
            <h1 class="page-title">Seller Package</h1>
        </div>
        <div class="ms-auto pageheader-btn">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="javascript:void(0);">Home</a></li>
                <li class="breadcrumb-item active" aria-current="page">Seller Package</li>
            </ol>
        </div>
    ...
    <div class="row my-2">
        <div class="text-end">
            <a href="{{route('sellerPackage.create')}}" class="btn btn-success my-1 float-end mx-2 text-center">Create New Package</a>
        </div>
    </div>
    <!--row-->
            ...
            <div class="col-lg-8 text-center col-xl-4 col-md-8 col-sm-12">
                <div class="card p-3 {{$package->status == 1 ? '':'bg-danger-transparent'}} pricing-card ">
                    <div class="card-header d-block text-justified pt-2">
                        ...
                        <p class="text-justify font-weight-semibold mb-1"> <span class="text-30 me-2">{{$currency->symbol ?? ''}}</span><span class="text-30 me-1">{{$package->amount}}.00</span> / <span class="text-20 me-1">{{$package->duration}} Days</span></p>
                    </div>
                    <div class="card-body pt-2">
                        <ul class="text-justify pricing-body ps-0">
                            <li class="mb-4">
                                ...
                                <strong> Product Upload Limit: {{$package->product_upload}}</strong>
                            </li>
                            <li class="mb-6 text-muted ">
                                ...
                                <strong class=""> {{$package->status == 1 ? 'Active':'Inactive'}}</strong>
                            </li>
                        </ul>
                        <div class="d-flex justify-content-center">
                            <a href="{{route('sellerPackage.edit',$package->id)}}" class="btn btn-sm mx-1 px-5 rounded-2 btn-success">
                                <span class="ms-4 me-4">Edit</span>
                            </a>
                            <a href="#" class="rounded-2 {{$package->status == 1 ? 'btn-outline-warning btn-danger':'btn-danger'}}">
                                <form class="" action="{{route('sellerPackage.destroy',$package->id)}}" method="post">
                                    @csrf
                                    @method('DELETE')
