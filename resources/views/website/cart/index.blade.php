                    ...
                    <div class="all-unit">
                        <form action="{{route('cart.update')}}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="totall-units">
                                ...
                                @foreach($products as $key=> $product)
                                    @php($pro =   App\Models\Product::find($product->product_id))
                                    <div class="row" style="border-bottom: 1px solid lightgray; margin-top: 20px; margin-bottom: 20px; align-items: center; padding-bottom: 16px;">
                                        ...
                                        <div class="unit1 col-lg-4 col-md-3 col-sm-3 col-sm-12 col-xs-12">
                                            ...
                                            <div class="protx">
                                                <p><a class="nav-link" href="{{route('product.details',$pro->slug)}}">{{$product->name}}</a></p>
                                        ...
                                        <div class="unit2 col-lg-2 col-md-2 col-sm-2 col-sm-3 col-xs-6">
                                            ...
                                            <div class="pro-rate">
                                                <p>{{$currency->symbol ?? '$' }} {{number_format($product->price)}}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="unit4 col-lg-2 col-md-2 col-sm-2 col-sm-3 col-xs-6">
                                            ...
                                            <div class="pro-rate"><p> {{$currency->symbol ?? '$' }}{{((double)$product->price * $product->qty ?? '')}}</p></div>
                                        </div>
                                        <div class="decrease unit5 col-lg-1 col-md-1 col-sm-1 col-sm-1 col-xs-1">
                                            <div class="pro-ratee" style="display: none;"><p>Actions </p></div>
                                            <form action="{{route('cart.delete',['rowId'=>$product->id])}}" method="POST" id="cart_delete">
                                                <div class="pro-del">
                                                    @csrf
                                                    @method('delete')
                                                    <button type="button"  onclick="document.getElementById('cart_delete').submit()"  class="btn btn-sm btn-danger">
                                    ...
                                    @php($sum = $sum + (double)$product->price*(int)$product->qty)
                                @endforeach
                        ...
                        <div class="order-summary" style="border-bottom: 1px solid lightgray; margin-top: 20px;">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-3 col-md-3 col-sm-2 col-xs-6">
                                    <div class="shiping-t">
                                        <p>{{$currency->symbol ?? '$' }}{{number_format($shipping_cost)}}</p>
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-3 col-md-3 col-sm-4 col-xs-6">
                                    <div class="shiping-tt">
                                        <p> {{$currency->symbol ?? '$' }}{{number_format($sum+$shipping_cost+$tax)}}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
