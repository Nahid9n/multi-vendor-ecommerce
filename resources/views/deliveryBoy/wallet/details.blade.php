...
@section('title','Payment Details Page')
@section('body')
    <!-- PAGE-HEADER -->
                ...
                <div class="card-body">
                    <div class="clearfix">
                        <div class="float-start">
                            <h3 class="card-title mb-0">#payment-{{$withdraw->payment_id}}</h3>
                        </div>
                        <div class="float-end">
                    ...
                    <div class="table-responsive push">
                        <table class="table table-bordered table-hover mb-0 text-nowrap border-bottom">
                            ...
                            <tr class="">
                                <th>Payment</th>
                                <td><span class="{{$withdraw->status == 0 ? 'bg-warning text-dark':''}}{{$withdraw->status == 1 ? 'bg-success text-white':''}}{{$withdraw->status == 2 ? 'bg-danger text-white':''}} p-1">{{$withdraw->status == 0 ? 'Pending':''}}{{$withdraw->status == 1 ? 'Done':''}}{{$withdraw->status == 2 ? 'Cancel':''}}</span></td>
