                    ...
                    <div class="card">
                                ...
                                <div class="card-body">
                                        ...
                                        <table class="table  border text-nowrap text-md-nowrap">
                                            ...
                                            @foreach($orderDetails as $order)
                                                <tr>
                                                    ...
                                                    <td class="text-end">{{ $order->product_price * $order->product_qty }} .{{$currency->symbol ?? ''}}</td>
                                    ...
                                    <div class="text-start my-3">
                                        <p><span class="fw-bold">Delivery Date : </span><span class="p-1 bg-info rounded-2">{{$orderss->delivery_date}}</span></p>
                                    ...
                                    <div class="my-3 d-flex">
                                        ...
                                        <form {{$orderss->delivery_status == 3 ?'hidden':''}}{{$orderss->delivery_status == 4 ?'hidden':''}} class="{{$orderss->deliveryBoy_pickup == 1 ?'disabled':''}}" action="{{route('deliveryBoy.confirm.pickup',$orderss->id)}}" method="post" >
                                            @csrf
                                        ...
                                        <form {{$orderss->delivery_status == 3 ?'hidden':''}}{{$orderss->delivery_status == 4 ?'hidden':''}} class="{{$orderss->deliveryBoy_pickup == 2 ?'disabled':''}}" action="{{route('deliveryBoy.cancel.request.pickup',$orderss->id)}}" method="post">
                                            @csrf
